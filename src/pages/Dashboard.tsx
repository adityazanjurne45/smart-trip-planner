...
import { StatCardSkeleton, TripCardSkeleton } from "@/components/ui/SkeletonCard";
...
const Dashboard = () => {
  ...
  const stats = [
    { icon: MapPin, label: "Trips Planned", value: pastTrips?.length || 0, color: "text-primary", bgColor: "bg-primary/10" },
    { icon: Clock, label: "Days Traveled", value: pastTrips?.reduce((acc, trip) => acc + trip.duration, 0) || 0, color: "text-accent", bgColor: "bg-accent/10" },
    { icon: Wallet, label: "Avg Budget", value: pastTrips?.length ? `$${Math.round(pastTrips.reduce((acc, trip) => acc + trip.budget, 0) / pastTrips.length)}` : "$0", color: "text-travel-gold", bgColor: "bg-travel-gold/10" },
    { icon: TrendingUp, label: "Avg Rating", value: pastTrips?.filter(t => t.rating).length ? (pastTrips.filter(t => t.rating).reduce((acc, trip) => acc + (trip.rating || 0), 0) / pastTrips.filter(t => t.rating).length).toFixed(1) : "N/A", color: "text-travel-forest", bgColor: "bg-travel-forest/10" },
  ];
    ...
    <div className="min-h-screen bg-secondary/20">
      ...
      <main className="container mx-auto px-4 pt-24 pb-12">
        <div className="max-w-5xl mx-auto">
          ...
          {/* Quick Action Cards */}
          <div className="grid md:grid-cols-2 gap-4 mb-8 animate-fade-in" style={{ animationDelay: "0.05s" }}>
            <Link to="/plan-trip" className="group">
              <Card className="bg-gradient-to-br from-primary/10 to-accent/5 border-primary/20 hover:border-primary/40 transition-all duration-300 hover:shadow-medium hover:-translate-y-0.5 cursor-pointer h-full">
            ...
            <Link to="/my-trips" className="group">
              <Card className="bg-gradient-to-br from-accent/10 to-travel-gold/5 border-accent/20 hover:border-accent/40 transition-all duration-300 hover:shadow-medium hover:-translate-y-0.5 cursor-pointer h-full">
                <CardContent className="p-6 flex items-center gap-4">
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-8 animate-fade-in" style={{ animationDelay: "0.1s" }}>
            {profileLoading ? (
              ...
              stats.map((stat) => (
                <Card key={stat.label} className="bg-card border-border shadow-soft hover:shadow-medium hover:-translate-y-0.5 transition-all duration-300">
            ...
            {profileLoading ? (
              ...
              <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-4">
                {pastTrips.slice(0, 3).map((trip) => (
                  <Card key={trip.id} className="border-border shadow-soft hover:shadow-medium hover:-translate-y-0.5 transition-all duration-300">
                    <CardContent className="p-5">
                      <div className="flex items-start justify-between mb-3">
                        <div className="flex items-center gap-2">
                          ...
                          <div>
                            <p className="font-medium text-foreground text-sm truncate max-w-[140px]">{trip.destination}</p>
                            <p className="text-xs text-muted-foreground">From {trip.boarding_point}</p>
                          </div>
                        </div>
                      </div>
                      <div className="flex items-center gap-3 text-xs text-muted-foreground">
                        <span className="flex items-center gap-1"><Calendar className="w-3 h-3" />{trip.duration}d</span>
                        <span className="flex items-center gap-1"><Wallet className="w-3 h-3" />${trip.budget}</span>
          ...
          {/* Achievements */}
          {pastTrips && (
            <ErrorBoundary fallbackMessage="Could not load achievements">
          ...
          {/* Travel Analytics */}
          {pastTrips && pastTrips.length > 0 && (
            <ErrorBoundary fallbackMessage="Could not load analytics">
